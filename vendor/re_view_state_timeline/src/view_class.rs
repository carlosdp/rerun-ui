...
const DEFAULT_FOLLOW_WINDOW_SECONDS: f64 = 10.0;
const DEFAULT_FOLLOW_WINDOW_STEPS: f64 = 200.0;
const FOLLOW_CURSOR_POSITION: f64 = 0.9;
...
impl ViewClass for StateTimelineView {
    ...
    fn ui(
        ...
        let latest_at = query.latest_at.as_i64();
        let current_time = TimeReal::from(if latest_at == i64::MAX {
            data_max
        } else {
            latest_at as f64
        });
        let default_time_span = default_time_span(ctx.time_ctrl.time_type());

        if ctx.time_ctrl.play_state() == PlayState::Following {
            let data_span = (data_max - data_min).abs();
            let time_spanned = state
                .time_view
                .map_or(default_time_span, |time_view| time_view.time_spanned)
                .max(default_time_span)
                .max(data_span);
            state.time_view = Some(following_time_view(current_time, time_spanned));
        } else if state.time_view.is_none() {
            let data_span = (data_max - data_min).abs();
            let time_spanned = data_span.max(default_time_span);
            let range_end = data_max.max(current_time.as_f64());
            state.time_view = Some(following_time_view(
                TimeReal::from(range_end),
                time_spanned,
            ));
        }
...
fn default_time_span(time_type: Option<TimeType>) -> f64 {
    match time_type {
        Some(TimeType::Sequence) => DEFAULT_FOLLOW_WINDOW_STEPS,
        Some(TimeType::DurationNs | TimeType::TimestampNs) | None => {
            DEFAULT_FOLLOW_WINDOW_SECONDS * 1_000_000_000.0
        }
    }
}

fn following_time_view(current_time: TimeReal, time_spanned: f64) -> TimeView {
    TimeView {
        min: TimeReal::from(current_time.as_f64() - (time_spanned * FOLLOW_CURSOR_POSITION)),
        time_spanned,
    }
}
