...
impl SpatialView3D {
    pub fn view_3d(
        ...
        let pointer_pos_ui = response.hover_pos().or_else(|| {
            if response.is_pointer_button_down_on() || response.dragged() || response.drag_stopped()
            {
        ...
        let (response, picking_config) = if let Some(pointer_pos_ui) = pointer_pos_ui {
